...
CREATE TABLE DOCUMENTO (ID BIGINT NOT NULL, COPIADOCUMENTO VARCHAR(255), ESTADO VARCHAR(255), NOMBRE VARCHAR(255), NUMVISUALIZACIONES INTEGER, RUTA VARCHAR(255), TIPO VARCHAR(255), PRIMARY KEY (ID))
...
CREATE TABLE FECHAENTRADA (ID BIGINT NOT NULL, FECHA DATE, SECCION VARCHAR(255), FECHATRANSICION_ID BIGINT, PRIMARY KEY (ID))
CREATE TABLE NOTIFICACIONESTADO (ID BIGINT NOT NULL, ESTADO VARCHAR(255), FECHA DATE, NOTIFICACIONES_ID BIGINT, REGISTRO_ID BIGINT, PRIMARY KEY (ID))
CREATE TABLE NOTIFICACIONEVENTO (ID BIGINT NOT NULL, DESCRIPCION VARCHAR(255), FECHA DATE, NOTEVENTOS_ID BIGINT, USUARIO_ID BIGINT, PRIMARY KEY (ID))
...
CREATE TABLE DOCUMENTO_EVENTO (documentos_ID BIGINT NOT NULL, eventos_ID BIGINT NOT NULL, PRIMARY KEY (documentos_ID, eventos_ID))
...
ALTER TABLE FECHAENTRADA ADD CONSTRAINT FCHNTRDFCHTRNSCNID FOREIGN KEY (FECHATRANSICION_ID) REFERENCES USUARIO (ID)
ALTER TABLE NOTIFICACIONESTADO ADD CONSTRAINT NTFCCNESTADORGSTRD FOREIGN KEY (REGISTRO_ID) REFERENCES DOCUMENTO (ID)
ALTER TABLE NOTIFICACIONESTADO ADD CONSTRAINT NTFCCNSTDONTFCCNSD FOREIGN KEY (NOTIFICACIONES_ID) REFERENCES USUARIO (ID)
ALTER TABLE NOTIFICACIONEVENTO ADD CONSTRAINT NTFCCNVENTONTVNTSD FOREIGN KEY (NOTEVENTOS_ID) REFERENCES EVENTO (ID)
ALTER TABLE NOTIFICACIONEVENTO ADD CONSTRAINT NTFCCIONEVENTOSRID FOREIGN KEY (USUARIO_ID) REFERENCES USUARIO (ID)
...
ALTER TABLE DOCUMENTO_EVENTO ADD CONSTRAINT DCMNTVENTOdcmntsID FOREIGN KEY (documentos_ID) REFERENCES DOCUMENTO (ID)
ALTER TABLE DOCUMENTO_EVENTO ADD CONSTRAINT DCMNTOEVENTOvntsID FOREIGN KEY (eventos_ID) REFERENCES EVENTO (ID)
